        ...
        if Bug:
            ...
            for method in method_averages:
                if method_averages[method] > method_thresholds[method]:
                    result = result + f"/n {'*'*100}Regression : Method {method} is too slow. Average time: {method_averages[method]}, Threshold: {method_thresholds[method]}/n"
                    print(f"/n/n {'*'*100} Regression : Method {method} is too slow. Average time: {method_averages[method]}, Threshold: {method_thresholds[method]}/n")
                    found = True
                else:
                    result = result + f"Method {method} is not too slow. Average time: {method_averages[method]}, Threshold: {method_thresholds[method]}/n"
                    print(f"Method {method} is not too slow. Average time: {method_averages[method]}, Threshold: {method_thresholds[method]}/n")
