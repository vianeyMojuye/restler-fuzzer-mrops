...
class MropDisjointChecker(CheckerBase):
    """ Checker For Disjoint MROP Operation """
    ...
    def _modif(self, method):
        ...
        # print("POST->POST   MR Disjoint : OK" if len(r[0]) == 0 else r[0])
        # print(f"POST -> {method}   MR Disjoint : OK" if len(r[1]) == 0 else r[1])

        r = dependencies.compare_for_disjoint_mrop(dependencies.equivalence_post_codes,dependencies.equivalence_get_codes,method)
        
        if len(dependencies.equivalence_post_codes)>= 2 :
            print("POST->POST   MR Disjoint : OK" if len(r[0]) == 0 else r[0])
        if len(dependencies.equivalence_get_codes)>= 2 :
            print(f"GET -> {method}   MR Disjoint : OK" if len(r[1]) == 0 else r[1])
